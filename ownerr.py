...
def food_sales_(orders)->dict:
    ...
    for order in orders:
        for item in order["items"]:
            food_sales_price[item["name"]] += (item["price"]*item["quantity"])
...
class Owner:
    ...
    def view_orders(self):
        if not self.orders:
            console.print("There is no order.")
            return
        rows=[]
        for food_index,order in enumerate(self.orders):
            ...
            row_item=[[item["name"],item['price'],item['quantity'],item['subtotal']]for item in order['items']]
